...
public class Piano extends LinearLayout {
	...
	public Piano(Context context, AttributeSet attrs) {
		...
		TypedArray array = context.obtainStyledAttributes(attrs,
				R.styleable.piano);
		mWidth = (int) array.getDimension(R.styleable.piano_p_width, 0);
		mHeight = (int) array.getDimension(R.styleable.piano_p_height, 0);
		mMargin = (int) array.getDimension(R.styleable.piano_p_margin, 0);
		mHidden = (int) array.getDimension(R.styleable.piano_p_hidden, 0);
		mDuration = array.getInteger(R.styleable.piano_p_duration, 0);
		activeIndex = -1;
		Log.d(TAG, "mWidth=" + mWidth + " mHeight=" + mHeight + " mMargin="
				+ mMargin + " mHidden=" + mHidden + " mDuration=" + mDuration);
		array.recycle();
	}
	...
	private void getAllPianoLoc() {
		...
		for (int i = 0; i < count; i++) {
			...
			mPL.add(new PianoLocation(piano.getLeft(), piano.getTop(), piano
					.getRight(), piano.getBottom()));
		}
	}

	/*
	 * 1. 获取所有子组件的位置信息 2. 根据触摸位置确定是否触摸到子组件上，若没有则直接返回 3.
	 * 查看触摸子组件是否已经处于激活状态，若是则直接返回 4. 调用组件恢复动画使上一次激活组件回到初始状态 5. 确定要激活组件 6.
	 * 调用组件激活动画 7. 返回激活组件索引
	 */
